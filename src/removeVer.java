...
public class removeVer {
    //垂直方向上寻找最小能量路径，以达到增宽的目的
    public static BufferedImage imag;
    public static void main(String[] args) {
        BufferedImage im=imag;
        execute(im);
    }
    public removeVer(BufferedImage i){
        imag=i;
    }
    public static void execute(BufferedImage image){
        try {
            ...
            int cut=in.nextInt();
            int newWidth = image.getWidth() - cut; // 要缩小成的宽度
            BufferedImage resultImage = seamCarving(image, newWidth);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static BufferedImage seamCarving(BufferedImage image, int newWidth) {
    ...
    private static int[][] calculateEnergy(BufferedImage image) {
        int width = image.getWidth();
        int height = image.getHeight();
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                energy[y][x] = calculatePixelEnergy(image, x, y);
            }
        }
    ...
    private static BufferedImage removeVerticalSeam(BufferedImage image, int[] seam) {
        ...
        for (int x = 0; x < width; x++) {
            ...
            for (int y = 0; y < height; y++) {
                if (y != seam[x]) {
                    //newImage.setRGB(x, k, Color.RED.getRGB());
