...
public class extendVer {
    ...
    public static void execute(BufferedImage image){
        try {
            ...
            System.out.println("Enter width(an integer) to be extended:");
            int cut=in.nextInt();
            int newWidth = image.getWidth() + cut; // 要扩展成的宽度

            seamCarving(image, newWidth);
            imag=transpose(imag);
            File output = new File("output4.jpg");
    ...
    public static void seamCarving(BufferedImage image,int newWidth) {
    ...
    public static int[][] calculateEnergy(BufferedImage image) {
        int width = image.getWidth();
        int height = image.getHeight();
    ...
    public static int calculatePixelEnergy(BufferedImage image, int x, int y) {
        int width = image.getWidth();
        int height = image.getHeight();
